    ...
    <div class="container px-4 py-6 mx-auto">
        <div class="flex flex-col md:flex-row items-center justify-between mb-6">
            ...
            <a href="{{ route('admin.routes.create') }}"
                class="mt-3 md:mt-0 flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all shadow-md">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($routes as $route)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $route->destination }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $route->distance ? $route->distance . ' km' : 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $route->duration }} menit
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Rp
                                    {{ number_format($route->base_price, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div class="text-xs space-y-1">
                                        <div>Motor: <span class="font-medium">Rp
                                                {{ number_format($route->motorcycle_price, 0, ',', '.') }}</span></div>
                                        <div>Mobil: <span class="font-medium">Rp
                                                {{ number_format($route->car_price, 0, ',', '.') }}</span></div>
                                        <div>Bus: <span class="font-medium">Rp
                                                {{ number_format($route->bus_price, 0, ',', '.') }}</span></div>
                                        <div>Truk: <span class="font-medium">Rp
                                                {{ number_format($route->truck_price, 0, ',', '.') }}</span></div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @elseif($route->status == 'WEATHER_ISSUE')
                                        <span
                                            class="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">Masalah
                                            Cuaca</span>
                                        @if ($route->status_expiry_date)
                                            <div class="mt-1 text-xs text-gray-500">
                                                Sampai:
                                                {{ \Carbon\Carbon::parse($route->status_expiry_date)->format('d M Y H:i') }}
                                            </div>
                                        @endif
                                    @endif
                                </td>
