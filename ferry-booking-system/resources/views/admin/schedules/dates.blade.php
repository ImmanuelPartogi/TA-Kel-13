...
@section('content')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <dl class="grid grid-cols-4 md:grid-cols-3 gap-x-4 gap-y-2">
                            ...
                            <dd class="col-span-3 md:col-span-2 text-sm text-gray-900">{{ $schedule->route->origin }} -
                                {{ $schedule->route->destination }}</dd>
                            ...
                            <dd class="col-span-3 md:col-span-2 text-sm text-gray-900">{{ $schedule->departure_time }} -
                                {{ $schedule->arrival_time }}</dd>
                        </dl>
                    </div>
                    <div>
                        <dl class="grid grid-cols-4 md:grid-cols-3 gap-x-4 gap-y-2">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <div class="flex items-center">
                                            <span class="mr-2">{{ $date->passenger_count }} /
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-xs text-gray-500">
                                        <div class="flex flex-col space-y-1">
                                            <div class="flex items-center justify-between">
                                                <span>Motor:</span>
                                                <span class="font-medium">{{ $date->motorcycle_count }} /
                                                    {{ $schedule->ferry->capacity_vehicle_motorcycle }}</span>
                                            </div>
                                            <div class="flex items-center justify-between">
                                                <span>Mobil:</span>
                                                <span class="font-medium">{{ $date->car_count }} /
                                                    {{ $schedule->ferry->capacity_vehicle_car }}</span>
                                            </div>
                                            <div class="flex items-center justify-between">
                                                <span>Bus:</span>
                                                <span class="font-medium">{{ $date->bus_count }} /
                                                    {{ $schedule->ferry->capacity_vehicle_bus }}</span>
                                            </div>
                                            <div class="flex items-center justify-between">
                                                <span>Truk:</span>
                                                <span class="font-medium">{{ $date->truck_count }} /
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @if ($date->modified_by_schedule)
                                            <span
                                                class="mt-1 px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-800 text-white">
                                                Diubah Oleh Jadwal
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex space-x-1 justify-end">
                                            <!-- Tombol Edit -->
                                            <button type="button"
                                                class="text-white bg-blue-600 hover:bg-blue-700 rounded-full p-1.5 inline-flex items-center justify-center editDateBtn"
                                                data-bs-toggle="modal" data-bs-target="#editDateModal"
                                                data-id="{{ $date->id }}"
                                            ...
                                            <!-- Tombol Hapus -->
                                            <form
                                                action="{{ route('admin.schedules.destroy-date', [$schedule->id, $date->id]) }}"
                                                method="POST"
                                                onsubmit="return confirm('Apakah Anda yakin ingin menghapus tanggal jadwal ini?');"
                                                class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="text-white bg-red-600 hover:bg-red-700 rounded-full p-1.5 inline-flex items-center justify-center">
    ...
    <!-- Add Date Modal -->
    <div class="fixed inset-0 overflow-y-auto hidden" id="addDateModal" aria-labelledby="addDateModalLabel"
                                ...
                                <form action="{{ route('admin.schedules.store-date', $schedule->id) }}" method="POST">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <select
                                            ...
                                            id="date_type" name="date_type" required>
                                    ...
                                    <div class="mb-4 p-3 bg-blue-50 rounded-md">
                                        ...
                                        <p class="text-xs text-blue-600 mt-1">
                                            <i>Catatan: Jika menggunakan rentang tanggal, hanya tanggal yang sesuai dengan
                                                hari operasi yang akan dibuat.</i>
                                        </p>
                                    </div>
                                    ...
                                    <div class="mb-4 hidden" id="endDateContainer">
                                        ...
                                        <input type="date"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                                            id="end_date" name="end_date" min="{{ date('Y-m-d') }}">
                                    </div>
                                    ...
                                    <div class="mb-4 hidden" id="addExpiryDateContainer">
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">Isi jika status akan berakhir pada waktu
                                            tertentu. Khusus untuk status Masalah Cuaca (WEATHER_ISSUE).</p>
                                    </div>
                            </div>
    ...
    <!-- Edit Date Modal -->
    <div class="fixed inset-0 overflow-y-auto hidden" id="editDateModal" aria-labelledby="editDateModalLabel"
        aria-hidden="true" tabindex="-1">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"
                data-bs-dismiss="modal"></div>
                            ...
                            <div class="mt-4">
                                <form id="editDateForm" action="" method="POST">
                                    ...
                                    <div class="mb-4 hidden" id="editReasonContainer">
                                        <label for="edit_status_reason"
                                            class="block text-sm font-medium text-gray-700 mb-1">
                                            Alasan Status
                                        </label>
                                        <input type="text"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                                            id="edit_status_reason" name="status_reason">
                                    </div>
                                    <div class="mb-4 hidden" id="editExpiryDateContainer">
                                        <label for="edit_status_expiry_date"
                                            class="block text-sm font-medium text-gray-700 mb-1">
                                            Tanggal Berakhir Status
                                        </label>
                                        <input type="datetime-local"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                                            id="edit_status_expiry_date" name="status_expiry_date">
                                        <p class="mt-1 text-xs text-gray-500">Isi jika status akan berakhir pada waktu
                                            tertentu. Khusus untuk status Masalah Cuaca (WEATHER_ISSUE).</p>
                                    </div>
                            </div>
                ...
                if (editBtns.length > 0 && editDateModal) {
                    editBtns.forEach(button => {
                        button.addEventListener('click', function(e) {
                            ...
                            // Perbaiki pengaturan URL action form
                            document.getElementById('editDateForm').action =
                                "{{ url('admin/schedules') }}/{{ $schedule->id }}/dates/" + id;
                            document.getElementById('editDateText').textContent =
                                `Tanggal: ${date}`;
