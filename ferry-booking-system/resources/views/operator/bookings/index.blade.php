                ...
                <form action="{{ route('operator.bookings.index') }}" method="GET" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <input type="text" name="booking_code" value="{{ request('booking_code') }}"
                            class="input input-bordered w-full" placeholder="Kode Booking">
                        <input type="text" name="user_name" value="{{ request('user_name') }}"
                            class="input input-bordered w-full" placeholder="Nama Pengguna">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white text-sm">
                        @forelse($bookings as $booking)
                            <tr>
                                ...
                                <td class="px-4 py-2">{{ $booking->user->name ?? 'Pengguna Tidak Ditemukan' }}</td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2">{{ \Carbon\Carbon::parse($booking->booking_date)->format('d M Y') }}
                                </td>
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    @endphp
                                    <span
                                        class="px-2 py-1 rounded text-xs font-medium {{ $badgeColors[$booking->status] ?? 'bg-gray-100 text-gray-800' }}">
