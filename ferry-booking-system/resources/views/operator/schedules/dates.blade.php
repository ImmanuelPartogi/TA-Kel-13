...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-blue-100 mt-1">{{ $schedule->route->origin }} - {{ $schedule->route->destination }} • {{ $schedule->departure_time->format('H:i') }}</p>
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-100">
                        ...
                        <p class="text-lg font-semibold text-gray-800">{{ $schedule->departure_time->format('H:i') }}</p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-4 border border-gray-100">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Alasan Status</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kapasitas Tersisa</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="w-full bg-gray-200 rounded-full h-2.5 mb-2">
                                <div class="bg-blue-600 h-2.5 rounded-full" style="width: {{ $passengerPercentage }}%"></div>
                            </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-y-2">
                            ...
                            <a href="{{ route('operator.schedules.edit-date', ['id' => $schedule->id, 'dateId' => $date->id]) }}"
                               class="flex w-full items-center justify-center px-2.5 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-150">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <button type="button" onclick="openDeleteModal('{{ $date->id }}', '{{ \Carbon\Carbon::parse($date->date)->format('d F Y') }}')"
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-6 py-4 text-sm text-gray-500 text-center">Tidak ada data tanggal jadwal</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="updateStatusModal{{ $date->id }}" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
        <div class="bg-white rounded-xl max-w-lg w-full mx-4 shadow-2xl transform transition-all">
            <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                ...
                <button type="button" class="text-gray-400 hover:text-gray-500" onclick="closeModal('updateStatusModal{{ $date->id }}')">
            ...
            <form action="{{ route('operator.schedules.update-date-status', ['id' => $schedule->id, 'dateId' => $date->id]) }}" method="POST">
                ...
                <div class="px-6 py-4">
                    <div class="mb-4">
                        <label for="status{{ $date->id }}" class="block text-sm font-medium text-gray-700 mb-1">Status:</label>
                        <select class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                            ...
                            <option value="active" {{ $date->status == 'AVAILABLE' ? 'selected' : '' }}>Tersedia</option>
                            <option value="inactive" {{ $date->status == 'UNAVAILABLE' ? 'selected' : '' }}>Tidak Tersedia</option>
                            <option value="suspended" {{ $date->status == 'CANCELLED' ? 'selected' : '' }}>Dibatalkan</option>
                    ...
                    <div class="mb-4">
                        <label for="statusReason{{ $date->id }}" class="block text-sm font-medium text-gray-700 mb-1">Alasan Status (Opsional):</label>
                        <textarea class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                    ...
                    <div class="mb-4 weather-expiry-group-{{ $date->id }}"
                        ...
                        <label for="statusExpiryDate{{ $date->id }}" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Berakhir Status:</label>
                        <input type="date" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                               id="statusExpiryDate{{ $date->id }}" name="status_expiry_date"
                               value="{{ $date->status_expiry_date ? \Carbon\Carbon::parse($date->status_expiry_date)->format('Y-m-d') : '' }}">
                    </div>
                </div>
    ...
    <!-- Modal Delete -->
    <div id="deleteModal{{ $date->id }}" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
        <div class="bg-white rounded-xl max-w-md w-full mx-4 shadow-2xl transform transition-all">
            <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <h3 class="text-lg font-medium text-gray-900">Konfirmasi Hapus</h3>
                <button type="button" class="text-gray-400 hover:text-gray-500" onclick="closeModal('deleteModal{{ $date->id }}')">
                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            ...
            <div class="px-6 py-4">
                <div class="text-sm text-gray-600">
                    ...
                    <div class="font-semibold text-gray-900 mt-1">{{ \Carbon\Carbon::parse($date->date)->format('d F Y') }}</div>
                </div>
            ...
            <div class="px-6 py-4 bg-gray-50 flex justify-end space-x-3">
                ...
                <form action="{{ route('operator.schedules.destroy-date', ['id' => $schedule->id, 'dateId' => $date->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
