...
@section('content')
                                    ...
                                    <tr>
                                        ...
                                        <td class="py-2 px-4">
                                            ...
                                            <div class="progress mb-2">
                                                <div class="progress-bar bg-primary" role="progressbar"
                                                    ...
                                                    aria-valuenow="{{ $passengerPercentage }}" aria-valuemin="0"
                                                    aria-valuemax="100"></div>
                                            </div>
                                            <small>{{ $schedule->ferry->capacity_passenger - $date->passenger_count }} dari
                                                {{ $schedule->ferry->capacity_passenger }} kursi tersedia</small>
                                        </td>
                                        ...
                                        <td class="py-2 px-4">
                                            <button type="button" class="btn btn-sm btn-primary" data-toggle="modal"
                                                data-target="#updateStatusModal{{ $date->id }}">
    ...
    @foreach ($dates as $date)
        <div class="modal fade" id="updateStatusModal{{ $date->id }}" tabindex="-1" role="dialog"
            aria-labelledby="updateStatusModalLabel{{ $date->id }}" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="updateStatusModalLabel{{ $date->id }}">Update Status Tanggal:
                            {{ \Carbon\Carbon::parse($date->date)->format('d F Y') }}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form
                        action="{{ route('operator.schedules.update-date-status', ['id' => $schedule->id, 'dateId' => $date->id]) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="status{{ $date->id }}" name="status" required>
                                    <option value="active" {{ $date->status == 'AVAILABLE' ? 'selected' : '' }}>Tersedia
                                    </option>
                                    <option value="inactive" {{ $date->status == 'UNAVAILABLE' ? 'selected' : '' }}>Tidak
                                        Tersedia</option>
                                    <option value="suspended" {{ $date->status == 'CANCELLED' ? 'selected' : '' }}>
                                        Dibatalkan</option>
                                    <option value="full" {{ $date->status == 'FULL' ? 'selected' : '' }}>Penuh</option>
                                    <option value="weather_issue"
                                        {{ $date->status == 'WEATHER_ISSUE' ? 'selected' : '' }}>Masalah Cuaca</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="statusReason{{ $date->id }}">Alasan Status (Opsional):</label>
                                <textarea class="form-control" id="statusReason{{ $date->id }}" name="status_reason" rows="3">{{ $date->status_reason }}</textarea>
                            </div>
                            <div class="form-group weather-expiry-group"
                                style="display: {{ $date->status == 'WEATHER_ISSUE' ? 'block' : 'none' }}">
                                <label for="statusExpiryDate{{ $date->id }}">Tanggal Berakhir Status:</label>
                                <input type="date" class="form-control" id="statusExpiryDate{{ $date->id }}"
