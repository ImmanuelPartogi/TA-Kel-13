...
import '../../screens/payment/webview_screen.dart';
...
class PaymentScreen extends StatefulWidget {
  ...
  const PaymentScreen({
    ...
    this.paymentType = 'virtual_account',
    this.bookingCode,
  }) : super(key: key);
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void _startStatusPolling() {
    ...
    if (bookingCode == null) {
      print('Error: bookingCode tidak tersedia');
      return;
    }
    ...
    _pollingTimer = Timer.periodic(Duration(seconds: 5), (timer) async {
      try {
        // Panggil method checkPaymentStatus tanpa menyimpan hasilnya
        await Provider.of<BookingProvider>(
          context,
          listen: false,
        ).checkPaymentStatus(bookingCode);

        // Ambil status terbaru dari provider setelah update
  ...
  void _openMidtransPayment(String snapToken, String bookingCode) {
  ...
  Future<void> _loadPaymentInstructions() async {
    ...
    try {
      // Pastikan _paymentMethod dan _paymentType tidak null dengan nilai default
      final String paymentMethod = _paymentMethod ?? 'bca';
      final String paymentType = _paymentType ?? 'virtual_account';
      ...
      if (paymentType == 'virtual_account' &&
          bookingProvider.currentBooking?.latestPayment != null) {
        _virtualAccountNumber =
            bookingProvider.currentBooking?.latestPayment?.virtualAccountNumber;
      }
  ...
  Widget _buildContent(BuildContext context, dynamic booking) {
    if (booking == null) {
      return const Center(child: Text('Data pemesanan tidak ditemukan'));
    }
    ...
    // Dengan nilai default untuk menghindari null
    final payment = booking.latestPayment;
    final String? virtualAccountNumber =
        payment?.virtualAccountNumber ?? _virtualAccountNumber;
    // Tambahkan nilai default 'bca' dan 'virtual_account' jika semua null
    final String activePaymentMethod =
        payment?.paymentMethod ??
        booking.paymentMethod ??
        _paymentMethod ??
        'bca'; // Default ke 'bca' jika semua null
    final String activePaymentType =
        payment?.paymentType ??
        booking.paymentType ??
        _paymentType ??
        'virtual_account'; // Default ke 'virtual_account' jika semua null

    // Buat list widget untuk children
    List<Widget> children = [];

    // Status dan waktu
    children.add(_buildStatusCard(context, booking, payment));
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Image.asset(
                      'assets/images/payment_methods/${activePaymentMethod == 'VIRTUAL_ACCOUNT' ? payment?.paymentMethod?.toLowerCase() ?? 'bni' : activePaymentMethod.toLowerCase()}.png',
                      fit: BoxFit.contain,
                      errorBuilder: (context, error, stackTrace) {
                        return Center(
                          child: Text(
                            activePaymentMethod.isNotEmpty
                                ? activePaymentMethod[0].toUpperCase()
                                : 'P',
                            style: TextStyle(
                  ...
                  Padding(
                    ...
                    child: SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          'Buka Aplikasi ${activePaymentMethod.toUpperCase()}',
                        ),
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                'Membuka aplikasi ${activePaymentMethod.toUpperCase()}...',
                              ),
                            ),
                          );
                        },
    ...
    // Tombol Bayar Sekarang jika ada snap token
    if (booking.status == 'PENDING' &&
        booking.latestPayment?.snapToken != null) {
      children.add(
        SizedBox(
          width: double.infinity,
          child: ElevatedButton.icon(
            ...
            onPressed: () {
              _openMidtransPayment(
                booking.latestPayment!.snapToken!,
                booking.bookingCode,
              );
            },
            style: ElevatedButton.styleFrom(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            if (payment != null && payment.paymentCode != null)
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(
