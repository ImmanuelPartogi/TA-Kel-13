...
class _RouteListScreenState extends State<RouteListScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final activeRoutes =
        routes?.where((route) => route.status == 'ACTIVE').toList() ?? [];

    final inactiveRoutes =
        routes?.where((route) => route.status != 'ACTIVE').toList() ?? [];

    // Filter routes based on search query
    final filteredActiveRoutes =
        activeRoutes.where((route) {
          ...
          final routeCodeMatch = route.routeCode.toLowerCase().contains(
            _searchQuery.toLowerCase(),
          );
          return originMatch || destinationMatch || routeCodeMatch;
        }).toList();

    final filteredInactiveRoutes =
        inactiveRoutes.where((route) {
          final originMatch = route.origin.toLowerCase().contains(
            _searchQuery.toLowerCase(),
          );
          final destinationMatch = route.destination.toLowerCase().contains(
                                            ...
                                            child:
                                                scheduleProvider.isLoading
                                                    ...
                                                    : filteredActiveRoutes
                                                        .isEmpty
                                                    ? _buildEmptyRoutes(
                                                      'Tidak ada rute aktif yang tersedia',
                                                    )
                                                    : ListView.builder(
                                                      padding:
                                          ...
                                          RefreshIndicator(
                                            ...
                                            child:
                                                scheduleProvider.isLoading
                                                    ...
                                                    : filteredInactiveRoutes
                                                        .isEmpty
                                                    ? _buildEmptyRoutes(
                                                      'Tidak ada rute nonaktif yang tersedia',
                                                    )
                                                    : ListView.builder(
                                                      padding:
                                                          const EdgeInsets.all(
          ...
          (context) => Dialog(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  // Header dengan info rute (tidak perlu diubah)
                  Row(
                    children: [
                      Container(
                        width: 50,
                  ...
                  // Bagian detail rute dengan harga yang diperbaiki
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        if (route.statusReason != null &&
                            route.statusReason!.isNotEmpty)
                        ...
                        // Bagian yang diperbaiki - menggunakan getVehiclePriceByType()
                        _buildDetailRow(
                          'Harga Motor',
                          'Rp ${route.getVehiclePriceByType('MOTORCYCLE').toStringAsFixed(0)}',
                        ),
                        _buildDetailRow(
                          'Harga Mobil',
                          'Rp ${route.getVehiclePriceByType('CAR').toStringAsFixed(0)}',
                        ),
                        _buildDetailRow(
                          'Harga Bus',
                          'Rp ${route.getVehiclePriceByType('BUS').toStringAsFixed(0)}',
                        ),
                        _buildDetailRow(
                          'Harga Truk',
                          'Rp ${route.getVehiclePriceByType('TRUCK').toStringAsFixed(0)}',
                        ),
                      ],
                    ),
                  ),

                  // Tombol-tombol di bawah (tidak perlu diubah)
                                ...
                                SnackBar(
                                  content: Row(
                                    children: [
                                      ...
                                      Text(
                                        'Rute ${route.origin} - ${route.destination} telah diaktifkan kembali',
                                      ),
                                    ],
                                  ),
