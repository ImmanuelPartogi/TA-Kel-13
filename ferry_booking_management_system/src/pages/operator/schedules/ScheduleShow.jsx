...
const ScheduleShow = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const datesResponse = await scheduleService.getUpcomingDates(id);
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
            ...
            <div className="p-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 flex flex-col">
                ...
                <span className="text-sm font-medium text-gray-800">
                  {(() => {
                    const hours = Math.floor(schedule.route.duration / 60);
                    const minutes = schedule.route.duration % 60;
                    let durationText = [];
                    ...
                    if (minutes > 0) {
                      durationText.push(`${minutes} menit`);
                    }
      ...
      <div className="mt-8">
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
                  ...
                  upcomingDates.map(date => (
                    <tr key={date.id} className="hover:bg-gray-50 transition-colors duration-150">
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-xs text-gray-500">{formatDayName(date.date)}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {date.status === 'ACTIVE' && (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900 font-medium">{date.booking_count || 0} booking</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <Link 
                          to={`/operator/bookings?schedule_id=${id}&date=${date.date}`}
          ...
          <div className="bg-gray-50 px-6 py-4 border-t border-gray-100 flex justify-between items-center">
            <span className="text-sm text-gray-500">Menampilkan {upcomingDates.length} dari {upcomingDates.length} tanggal mendatang</span>
            <Link 
              to={`/operator/schedules/${id}/dates`}
              className="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-800 transition-colors duration-150"
            >
